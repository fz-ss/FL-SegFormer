...
class JointTransform2D:
    ...
    def __init__(self, crop=(32, 32), p_flip=0.5, color_jitter_params=(0.1, 0.1, 0.1, 0.1),
        ...
        self.crop = crop  # none
        self.p_flip = p_flip  # p=0.5
        self.color_jitter_params = color_jitter_params  # none
        if color_jitter_params:
            self.color_tf = T.ColorJitter(*color_jitter_params)
        self.p_random_affine = p_random_affine  # none
        self.long_mask = long_mask  # ture
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = cv2.imread(os.path.join(self.output_path, image_filename[: -3] + "png"),
                          0)  # 0 means read the image as 8 bit and one channel

        mask[mask <= 127] = 0
        mask[mask > 127] = 1
        # correct dimensions if needed
        image, mask = correct_dims(image,
                                   mask)  # the dimensions of both image and mask should be 3 (H, W, 3) and (H, W, 1)
        # print(image.shape)
