...
class JointTransform2D:
    ...
    """
    '''
    JointTransform2D是一个用于对图像进行变换和增强的类。这个类的参数包括：
        img_size：输入图像的大小。
        crop：是否对图像进行裁剪。
        p_flip：进行水平翻转的概率。
        p_rota：进行旋转的概率。
        p_scale：进行缩放的概率。
        p_gaussn：进行高斯噪声添加的概率。
        p_contr：进行对比度调整的概率。
        p_gama：进行伽马校正的概率。
        p_distor：进行畸变的概率。
        color_jitter_params：颜色调整参数。
        long_mask：是否使用长掩模。
    '''
    def __init__(self, crop=(32, 32), p_flip=0.0, p_rota=0.0, p_scale=0.0, p_gaussn=0.0, p_contr=0.0,
    ...
    def __call__(self, image, mask):
        ...
        if np.random.rand() < self.p_gaussn:
            ...
            noise = np.random.normal(loc=0, scale=1, size=(iw, ih)) * ns
...
class ImageToImage2D(Dataset):
    ...
    Usage:
        ...
        2. With unet.model.Model wrapper, an instance of this object should be passed as train or validation
           datasets.
    这段文字是关于ImageToImage2D类的使用说明。具体来说，如果不使用unet.model.Model包装器，那么应该将ImageToImage2D类的一个实例传递
    给torch.utils.data.DataLoader。通过迭代这个实例，可以返回图像、掩码和图像文件名的元组。如果使用unet.model.Model包装器，则应
    将ImageToImage2D类的一个实例作为训练或验证数据集传递。1
    Args:
        ...
        joint_transform是一个数据增强变换，它是JointTransform2D类的一个实例。如果joint_transform为False，
        则会使用torchvision.transforms.ToTensor将图像和掩码转换为张量。one_hot_mask是一个布尔值，如果为True，
        则会将掩码以one-hot编码的形式返回
    """
#Callable 类型提示表示 joint_transform 是一个函数，它接受一些参数并返回一个值。
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=4,
                 one_hot_mask: int = False) -> None:
        self.dataset_path = dataset_path # data_path = "./data/"
        self.img_path = os.path.join(dataset_path, 'out')
        self.label_path = os.path.join(dataset_path, 'outlabel')
        self.classes = classes
        self.one_hot_mask = one_hot_mask
        id_list_file = os.path.join(dataset_path, 'ImageSets/Segmentation/{0}.txt'.format(split))
    ...
    def cvtColor(self,image):
        if len(np.shape(image)) == 3 and np.shape(image)[2] == 3:
            ...
            return image
#  #当我们使用 [] 运算符访问一个对象时，Python 会自动调用该对象的 __getitem__ 方法来获取对应的值。
    def __getitem__(self, i):
        id_ = self.ids[i]

        # jpg = Image.open(os.path.join(self.img_path, id_ + '.jpg'))
        # png = Image.open(os.path.join(self.label_path, id_ + '.png'))
        # jpg = self.cvtColor(jpg)
        # png = Image.fromarray(np.array(png))
        # h, w = 512, 512
        # iw, ih = jpg.size
        # image = jpg.resize((600, 400), Image.BICUBIC)
        # label = png.resize((600, 400), Image.NEAREST)
        # dx = int(self.rand(0, w - 600))
        # dy = int(self.rand(0, h - 400))
        ...
        image = cv2.imread(os.path.join(self.img_path, id_ + '.jpg'))
