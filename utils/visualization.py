...
def visual_segmentation(seg, image_filename, opt):
    img_ori = cv2.imread(os.path.join(opt.data_path + '/outlabel', image_filename))
    img_ori0 = cv2.imread(os.path.join(opt.data_path + '/outlabel', image_filename))
    overlay = img_ori * 0
    img_r = img_ori[:, :, 0]
    img_g = img_ori[:, :, 1]
    img_b = img_ori[:, :, 2]
    table = np.array([[0,0,128], [0, 128, 0], [0, 128, 128], [0, 0, 0]])
    seg0 = seg[0, :, :]

    for i in range(1, opt.classes):

        img_r[seg0 == i] = table[i + 1 - 2, 0]
        img_g[seg0 == i] = table[i + 1 - 2, 1]
        img_b[seg0 == i] = table[i + 1 - 2, 2]
